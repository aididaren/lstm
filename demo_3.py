...
def readData(column='high', n=30, all_too=True, index=False, train_end=-500):
    df = pd.read_csv("C:/Users/DK/Desktop/LSTM/lstm/sh300.csv", index_col=2)
    # 以日期为索引
    df.index = list(
    ...
    # df = pd.read_csv('C:/Users/DK/Desktop/LSTM/lstm/sh300.csv',encoding='utf-8',parse_dates=['trade_date'],date_parser=dateparse, index_col=0)
    ...
    # 生成训练数据
    df_generate_train = generate_data_by_n_days(
...
for i in range(n, len(df_all)):
    ...
    if i < test_index:
        generate_data_train.append(torch.squeeze(
            y).detach().cpu().numpy() * df_numpy_std + df_numpy_mean)
    else:
...
plt.plot(df_index[train_end:-500],
         generate_data_test[-600:-500], label='generate_test')
plt.legend()
plt.show()
